                    ...
                    let warn = async (mentioned) => {
                        ...
                        if (config.Moderation.AutoWarnPunishments && Object.keys(config.Moderation.AutoWarnPunishments).find(key => parseInt(key) == parseInt(warns.length))) {
                            ...
                            if (autoP[0] == 'ban') {
                                ...
                                await Utils.variables.db.update.roles.setSavedMuteRoles(message.member, JSON.stringify(message.member.roles.cache.filter(r => r.name !== "@everyone" && !r.tags.premiumSubscriberRole).map(r => r.id)));
                                await message.member.roles.remove(message.member.roles.cache.filter(r => r.name !== "@everyone" && !r.tags.premiumSubscriberRole));
                            ...
                            if (user) {
                                ...
                                message.channel.send(Utils.setupMessage({
                                    ...
                                    variables: [
                                        ...
                                        { searchFor: /{warning-count}/g, replaceWith: warns.length + 1 }
                                    ]
                                })).then(m => {
