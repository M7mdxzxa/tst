...
module.exports = async (bot, member, channel, autoDeleteMessages = false, delay = 10000, reply, predefinedPosition = undefined, buttonOptions) => {
    return new Promise(async resolve => {
        ...
        const sendToChannel = (embed, ephemeral = false, autoDelete = false) => {
            ...
            else if (buttonOptions?.ephemeral && buttonOptions?.interaction) buttonOptions.interaction.reply(Object.assign({}, embed, { ephemeral: true })).catch(console.error);
        ...
        channel.guild.channels.create(settings.Channel.Format.replace(/{username}/g, member.user.username).replace(/{id}/g, member.user.id).replace(/{tag}/g, member.user.tag), {
            ...
            const getFormattedPosition = name => Position_Keys.find(pos => pos.toLowerCase() == name.toLowerCase());
            ...
            if (!predefinedPosition) {
                ...
                if (!getFormattedPosition(predefinedPosition)) {
            ...
            if (predefinedPosition) done(getFormattedPosition(predefinedPosition));
            else getPosition();

            setTimeout(() => {
                if (!buttonOptions?.interaction?.replied && !buttonOptions?.interaction?.deferred) buttonOptions.interaction.deferUpdate();
            }, 2500);
        });
    });
};
