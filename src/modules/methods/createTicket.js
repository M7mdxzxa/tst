...
module.exports = async (bot, args, member, channel, autoDeleteMessages = false, delay = 10000, requireReason = config.Tickets.RequireReason, category_name = config.Tickets.Channel.Category, sendTicketCreatedMessage = config.Tickets.TicketCreatedMessage.Enabled, reply, isSlashCommand = false, buttonOptions) => {
    return new Promise(async resolve => {
        ...
        const sendToChannel = (embed, ephemeral = false) => {
            ...
            else if (buttonOptions?.ephemeral && buttonOptions?.interaction) buttonOptions.interaction.reply(Object.assign({}, embed, { ephemeral: true }));
