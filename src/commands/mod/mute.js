...
module.exports = {
    ...
    run: async (bot, messageOrInteraction, args, { prefixUsed, member, user, guild, reply }) => {
        return new Promise(async resolve => {
            ...
            await Utils.variables.db.update.roles.setSavedMuteRoles(targetUser, JSON.stringify(targetUser.roles.cache.filter(r => r.name !== "@everyone" && (r.tags ? !r.tags.premiumSubscriberRole : true)).map(r => r.id)));
            await targetUser.roles.remove(targetUser.roles.cache.filter(r => r.name !== "@everyone" && (r.tags ? !r.tags.premiumSubscriberRole : true)));
